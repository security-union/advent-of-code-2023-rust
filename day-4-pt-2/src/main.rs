...
fn explore_nodes(cards: &[Card]) -> u32 {
    let mut total = 0;
    for (i, card) in cards.iter().enumerate() {
        total = total + explore_clones(cards, i);
    }
    total
}

fn explore_clones(cards: &[Card], i: usize) -> u32 {
    if let Some(card) = cards.get(i) {
        let score = i + 1 + card.score as usize;
        let total = 1;
        let mut acc = 0;
        let ii = i+1;
        for j in ii..score {
            println!("exploring {} from {}", j, i);
            acc = acc + explore_clones(&cards, j);
        }        
        total + acc
    } else {
        0
